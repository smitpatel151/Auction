...
class ProgressBar(object):
    def __init__(
        ...
        self.pos = 0
        self.avg = []
        self.start = self.last_eta = timedemo.time()
    ...
    def is_fast(self):
        return timedemo.time() - self.start <= self.short_limit
    ...
    def make_step(self, n_steps):
        ...
        if (timedemo.time() - self.last_eta) < 1.0:
            return

        self.last_eta = timedemo.time()

        # self.avg is a rolling list of length <= 7 of steps where steps are
        # defined as time elapsed divided by the total progress through
        # self.length.
        if self.pos:
            step = (timedemo.time() - self.start) / self.pos
        else:
            step = timedemo.time() - self.start
